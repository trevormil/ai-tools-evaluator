...
import { METRICS, CATEGORY_LABELS, lensFor, type Category } from "@aix/core";
...
/**
 * The per-item one-pager (ticket 0081): every scored item's evaluation rendered
 * as a single spec sheet in the Test Bench language — the item-sized sibling of
 * /about. Pure read of the stored Evaluation JSON; pending items have no
 * evaluation, so they 404 rather than render an empty sheet.
 */

export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  const { slug } = await params;
  const item = getItemBySlug(slug);
  if (!item || item.scoreStatus === "pending") return { title: "Not found — AIx" };
  return {
    title: `${item.title} — one-pager — AIx`,
...
export default async function OnePagerPage({ params }: { params: Params }) {
  const { slug } = await params;
  const item = getItemBySlug(slug);
  if (!item || item.scoreStatus === "pending") notFound();
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-10">
      ...
      <section className="flex flex-col gap-4 pt-4">
        <p className="eyebrow">
          Spec sheet · {CATEGORY_LABELS[item.category as Category] ?? item.category} · evaluated{" "}
          {evaluatedAt} by {evaluation.evaluatedBy === "ai" ? evaluation.model : "human"}
        </p>
