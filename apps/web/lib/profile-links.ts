...
export function getProfileLinks(userId: string): ProfileLink[] {
  ...
  return rows.sort((a, b) => (order.get(a.kind as ProfileLinkKind) ?? 99) - (order.get(b.kind as ProfileLinkKind) ?? 99));
}
 ...
 */
export function setProfileLinks(userId: string, links: { kind: ProfileLinkKind; url: string }[]): ProfileLink[] {
  const db = getDb();
