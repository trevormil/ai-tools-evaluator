...
export function StackSection({ stack, isSelf, username }: { stack: StackEntry[]; isSelf: boolean; username: string }) {
  const grouped = STACK_STATUSES.map((status) => ({ status, entries: stack.filter((e) => e.status === status) })).filter(
    ...
    <section>
      ...
      {stack.length === 0 ? (
        <p className="text-sm text-neutral-500">
          {isSelf ? "Add the tools you actually run and your honest take on each." : "No tools in this stack yet."}
        </p>
      ) : (
        <div className="mt-4 flex flex-col gap-5">
          {grouped.map((g) => (
            <div key={g.status}>
              <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-neutral-500">{STACK_STATUS_LABELS[g.status]}</h3>
              <ul className="flex flex-col gap-2">
                {g.entries.map((e) => (
                  <li key={e.id} className="card flex flex-col gap-1 p-3">
                    ...
                    {e.take && <p className="text-sm text-neutral-700 dark:text-neutral-300">{e.take}</p>}
                  </li>
                ))}
              </ul>
            </div>
